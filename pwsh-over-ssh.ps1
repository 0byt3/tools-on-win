...
Function Resolve-SpaceSeparatedNames {
    ...
    $currentName = [System.Collections.ArrayList]::new();
    for ($i = 0; $i -lt $InputString.Length; $i++) {
        $char = $InputString[$i];
        if ($char -eq '"') {
            $currentName.Add($char) | Out-Null;
            $inQuotes = !$inQuotes;
            if (-not $inQuotes -and $currentName.Count -gt 0) {
                $names.Add(
                    $($currentName -join "")
                ) | Out-Null;
                $currentName.Clear() | Out-Null;
            }
        }
